            ...
            <InspectorControls key="${key}-settings">
                <PanelBody title="${label} Settings" initialOpen={true}>
                    ...
                    { attributes.${key} && typeof attributes.${key} === 'object' && attributes.${key}.url && (
                        <img src={ attributes.${key}.url } alt={ attributes.${key}.alt } style={{maxWidth: '100%', height: 'auto', marginTop: '10px'}} />
                    ) }
                </PanelBody>
            </InspectorControls>
        `,
        ...
        jsx: (key, label) => `
            <InspectorControls key="${key}-settings">
                <PanelBody title="${label} Settings" initialOpen={true}>
                    ...
                    />
                    { attributes.${key}?.url && (
                        <a href={ attributes.${key}.url } className="gk-btn-preview" target="_blank" rel="noopener noreferrer">
