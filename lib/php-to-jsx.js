...
function convertRenderPhpToJsx(phpContent) {
    ...
    // 4. Handle Simple Text / Number / Date / Time / Icon / Relational interpolations.
    jsx = jsx.replace(/<\?php echo (?:esc_html|esc_attr)\(\$(attributes|item)\['(.*?)'\].*?\); \?>/g,
        '{$1.$2}');

    // 5. Handle ContentEditor / Textarea (wp_kses_post)
    jsx = jsx.replace(/<\?php echo wp_kses_post\(\$(attributes|item)\['(.*?)'\].*?\); \?>/g,
        '<div dangerouslySetInnerHTML={{ __html: $1.$2 }} />');

    // 6. Handle Color (Inline styles)
    jsx = jsx.replace(/style=(?:'|")color:\s*\{attributes\.(.*?)\};(?:'|")/g,
    ...
    // Remove any remaining stray PHP tags just to be safe
