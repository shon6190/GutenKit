...
const ComponentEditorApp = ({
  ...
  const handleSave = () => {
    ...
    const data = {
      ...
      nonce: blockFactoryEditorData.nonce,
      // Retrieve nonce from PHP
  ...
  const renderRepeaterSettings = (field, fieldIndex, updateFieldProp, removeFieldAtIndex) => {
    ...
    createElement('h4', null, 'Add New Sub-Field'), createElement('div', {
      style: {
        display: 'flex',
        flexWrap: 'wrap',
        gap: '5px',
    ...
    }, FIELD_TYPES.filter(t => t.value !== 'repeater' && t.value !== 'relational').map(type => createElement(Button, {
      isSecondary: true,
      onClick: () => addSubField(type.value)
    }, type.label))),
    // 2. List of Existing Sub-Fields
    createElement('h4', null, `Current Sub-Fields (${subFields.length})`), subFields.length > 0 ? subFields.map((subField, subIndex) => renderSubFieldSettings(subField, subIndex)) : createElement('p', null, 'No sub-fields defined. Add one above.'));
  };
  ...
  const renderSettings = () => {
    ...
    if (!selectedField) return createElement("p", null, "Select a field on the left to edit its properties.");
    const index = fields.indexOf(selectedField);
    ...
    }, "Delete Field")), selectedField.type === 'repeater' && renderRepeaterSettings(selectedField, index, updateField, removeField));
  };
  ...
  }, createElement('h3', null, 'Component Palette'), FIELD_TYPES.map(type => createElement(Button, {
    isSecondary: true,
    style: {
      display: 'block',
      width: '100%',
  ...
  fields.map((field, index) => createElement('div', {
    key: index,
    style: {
      padding: '10px',
      marginBottom: '8px',
  ...
  }, `${field.label} (${field.type})`)), createElement('hr', null),
  ...
  createElement(Button, {
    ...
    onClick: handleSave,
    disabled: isSaving
  }, isSaving ? 'Saving...' : 'Save Structure & Regenerate Code'), message && createElement('p', {
    style: {
      marginTop: '15px',
      fontWeight: 'bold'
    }
...
jQuery(document).ready(function () {
  ...
  if (rootElement && typeof blockFactoryEditorData !== 'undefined') {
    wp.element.render(createElement(ComponentEditorApp, {
